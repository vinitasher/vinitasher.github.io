# Home Office – Moving Checklist

> **Room Code:** `7 – Grey – Office`
> Write `7 – Office` on every office box label.
>
> **Dec 5 (You):** Pack and personally move sensitive electronics and documents.
> **Dec 6 (Movers @ 2 PM):** Movers handle furniture and labeled office boxes.

## Furniture Placement (2561 Middlefield)
- **Bookshelf** goes in the **Office** (not guest or kids rooms).
- Large **computer desk** and the main **TV** live in the **Office / media room**.

## Packing Checklist (Before / On Dec 5)
- [ ] Back up computers and important files
- [ ] Pack laptops and sensitive electronics to carry personally
- [ ] Label and bundle all cables
- [ ] Pack monitor(s) with padding and "FRAGILE" labels
- [ ] Pack office supplies (pens, notebooks, files)
- [ ] Pack reference books
- [ ] Pack decor and plants

## Unpacking & Setup Checklist (After Move to 2561 Middlefield)
- [ ] Decide desk placement (light, noise, background)
- [ ] Set up monitor, laptop stand, keyboard, and mouse
- [ ] Connect internet and test Wi-Fi strength
- [ ] Organize cables and label chargers
- [ ] Set up filing or storage system
- [ ] Create a clutter-free video call background

## Donate / Sell / Trash Notes
- [ ] Extra office chairs / furniture
- [ ] Old tech for e-waste

## Notes
- Ideal work hours / zones in house:
